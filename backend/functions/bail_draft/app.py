...
4. Facts: state the arrest date, charged section(s), and days in custody, using the exact figures given
...
def build_bail_draft_messages(case_data: dict) -> list:
    context = (
        ...
        f"Days in custody: {case_data.get('daysInCustody') or case_data.get('days_in_custody')}\n"
...
def lambda_handler(event, context):
    ...
    if not case_id:
        return {"statusCode": 400, "body": json.dumps({"error": "caseId required"})}
    ...
    if case_data.get("status") != "PROCESSED" or case_data.get("flag") == "NOT_ELIGIBLE":
        return {
            "statusCode": 400,
            "body": json.dumps({"error": "Case is not eligible for a bail draft — flag must be PAST_MAX, PAST_HALF, or PAST_THIRD"})
        }
    ...
    return {
        ...
        "body": json.dumps({"caseId": case_id, "draft": draft}),
    }