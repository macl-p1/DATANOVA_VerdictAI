...
- accused_name: the full name of the accused person as written in the document. If several people are accused, use the first one named. If no name is stated, use null.
- sections: array of charged legal sections, in the EXACT format "PREFIX#NUMBER" (e.g. "IPC#379", "BNS#303(2)"). Use IPC for Indian Penal Code references, BNS for Bharatiya Nyaya Sanhita references. Keep any sub-clause exactly as written, including brackets. Do not write "Section 379" or "IPC Section 379" — only "IPC#379".
...
- evidence: an object with one key per field above (accused_name, sections, arrest_date, in_custody, release_date, first_time_offender, other_pending_cases), each holding:
...
Example output:
{"accused_name": "Rajesh Yadav", "sections": ["IPC#379"], "arrest_date": "2019-03-15", "in_custody": true, "release_date": null, "first_time_offender": true, "other_pending_cases": false, "evidence": {"accused_name": {"value": "Rajesh Yadav", "source_quote": "the accused Rajesh Yadav, son of Ram Yadav", "confidence": 0.97}, "arrest_date": {"value": "2019-03-15", "source_quote": "arrested on 15th March 2019", "confidence": 0.95}}}
"""
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Hand the rules Lambda a already-decided NEEDS_REVIEW rather than a
        # half-parsed document. rules/app.py forwards any event that already
        # carries a flag instead of trying to validate it as Extracted.
        return {
            "flag": "NEEDS_REVIEW",
            "days_in_custody": None,
            "days_overdue": None,
